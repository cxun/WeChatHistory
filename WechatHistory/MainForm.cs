...
namespace WechatHistory
{
    public partial class MainForm : Form
    {
        ...
        private void ReleaseEmoji()
        {
            ...
            for (int i = 0; i < 105; i++)
                StreamToFile(m_strEmojiPath, i + ".png");
            StreamToFile(m_strEmojiPath, "emoji17ced3.png");
        }
        ...
        private void StreamToFile(string strPath, string strEmoji)
        {
            System.Reflection.Assembly asm = System.Reflection.Assembly.GetExecutingAssembly();
            string name = asm.GetName().Name;
            Stream stream = asm.GetManifestResourceStream(name + ".image.emoji." + strEmoji);
        ...
        private void tvFriends_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            wbHistory.Source = new System.Uri(strLastFile);
            setNaviEnable();
        ...
        public void SetHistory(FriendInfo info, int nPage)
        {
            int nPageCount = 0;
            ...
            m_strLastFriend = info.strUsrName;
            m_nCurPage = nPage;
            wbHistory.Source = new Uri(m_strTmpPath + "WechatHistory_" + m_nCurPage + ".html");
            setNaviEnable();
            m_nLastClickTab = tabControl.SelectedIndex;
        }
        ...
        private void btnBackward_Click(object sender, EventArgs e)
        {
            ...
            if (File.Exists(strWeb))
                wbHistory.Source = new Uri(strWeb);
            wbHistory.Focus();
            setNaviEnable();
        }
        ...
        private void btnFirst_Click(object sender, EventArgs e)
        {
            ...
            if (File.Exists(m_strTmpPath + "WechatHistory_1.html"))
                wbHistory.Source = new Uri(m_strTmpPath + "WechatHistory_1.html");
            wbHistory.Focus();
            setNaviEnable();
        }
        ...
        private void Awesomium_Windows_Forms_WebControl_Click(object sender, EventArgs e)
        {
            ...
            // 点击分享链接
            Uri target = ((Awesomium.Windows.Forms.WebControl)sender).TargetURL;
            if (target != null && target.ToString().CompareTo("about:blank") != 0)
            {
            ...
            try
            {
                strPicPath = wbHistory.ExecuteJavascriptWithResult(
        ...
        private void btnSearchHistory_Click(object sender, EventArgs e)
        {
            ...
            if (strSearch.Trim().Length == 0) return;
        ...
        private int SearchHistory(string strSearch, Dictionary<string, FriendInfo> friendList, ProgressForm fmProgress)
        {
            ...
            List<KeyValuePair<FriendInfo, KeyValuePair<int, string>>> foundList =
                new List<KeyValuePair<FriendInfo, KeyValuePair<int, string>>>();
            foreach (KeyValuePair<string, FriendInfo> kvp in friendList)
            {
                ...
                while (dr.Read())
                {
                    ...
                    if (strMessage.Contains(strSearch))
                    {
                        ...
                        KeyValuePair<FriendInfo, KeyValuePair<int, string>> kvpNode =
                            new KeyValuePair<FriendInfo, KeyValuePair<int, string>>(kvp.Value, kvpValue);
            ...
            foreach (KeyValuePair<FriendInfo, KeyValuePair<int, string>> kvpNode in foundList)
            {
                ListViewItem lvItem = new ListViewItem();
                lvItem.Text = "与“" + kvpNode.Key.strDisplayName + "”的聊天记录中：" + kvpNode.Value.Value;
                lvItem.Tag = kvpNode;
